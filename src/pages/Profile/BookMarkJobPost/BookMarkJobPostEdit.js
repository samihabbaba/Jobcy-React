...
const BookMarkJobPostEdit = (props) => {
  if (!props.location.state || !props.location.state.selectedJobDetails) {
    window.location.href = "/error404";
  }

  const { selectedJobDetails } = props.location.state;
  ...
  const jobId = selectedJobDetails.jobId;
  ...
  const removeSkill = (index) => {
    ...
    // Update the skills state with the modified array
  ...
  useEffect(() => {
    const getJobById = async () => {
      try {
        const jobResponse = await axiosInstance.get(`/job/${jobId}`); // Replace with the appropriate endpoint for getting job details
        setTitle(jobResponse.data.title);
        setJobDescription(jobResponse.data.jobDescription);
        setCategory(jobResponse.data.category)
        setEmployeeType(jobResponse.data.employeeType);
        setSalary(jobResponse.data.salary);
        setQualification(jobResponse.data.qualification);
        setLocation(jobResponse.data.location);
        setExperience(jobResponse.data.experience);
        setIndustry(jobResponse.data.industry);
        setPosition(jobResponse.data.position);
        setSkills(jobResponse.data.skills);
        setRequirements(jobResponse.data.requirements);
        setResponsibilities(jobResponse.data.responsibilities);
      } catch (error) {
          ...
          <Row>
            <Col lg={12}>
              <div className="bg-soft-primary p-3">
                <h5 className="mb-0 fs-17">Edit Job</h5>
              </div>
            </Col>
          </Row>
          <Form
              ...
              <Row className="row">
                ...
                <Col lg={6}>
                  <div className="mb-4">
                    ...
                    {responsibilities.map((responsibility, index) => (
                      <span key={index} className="badge bg-soft-success ms-2">
                        {responsibility}
                        <button
                ...
                <Col lg={6}>
                  <div className="mb-4">
                    ...
                    />
                    <button className="btn btn-blue mt-1" type="button" onClick={addSkill} >
                      Add
                    </button>
                ...
                <Col lg={12}>
                  <div className="d-flex flex-wrap align-items-start gap-1 justify-content-end">
                    ...
                    <button className="btn btn-blue" type="button" onClick={handleUpdateJob}>
                      Update Now <i className="mdi mdi-send"></i>
                    </button>
                  </div>
...
export default BookMarkJobPostEdit;